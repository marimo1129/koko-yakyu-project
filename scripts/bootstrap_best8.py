...
# このスクリプトは scripts/ 配下に置く前提（上の階層がリポジトリルート）
...
teams_headers = ["prefecture","team_name","result","prefectural_rank","seed","region","note"]
RESULT_SLOTS = [
    ("優勝", 1), ("準優勝", 2),
    ("ベスト4", 3), ("ベスト4", 4),
...
with teams_csv.open("w", newline="", encoding="utf-8") as f:
    ...
    for pref, region in REGION_MAP.items():
        for label, rank in RESULT_SLOTS:
            w.writerow({
                "prefecture": pref,
                "team_name": "",
                "result": label,
...
with area_csv.open("w", newline="", encoding="utf-8") as f:
    ...
    for area in AREAS:
        for _ in range(8):
            w.writerow({"area": area, "team_name": "", "prefecture": "", "result": "", "round_exit": "", "note": ""})
